...
const Hero: React.FC = () => {    return (    <section id="ana-sayfa" className="relative min-h-screen w-full flex items-center justify-center overflow-hidden pt-16 sm:pt-20 md:pt-24">
      ...
      </div>{/* Content */}      <div className="container mx-auto px-4 xs:px-5 sm:px-6 z-10 relative">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center mt-8 xs:mt-4 sm:mt-0">
          <div className="flex flex-col space-y-4 xs:space-y-5 sm:space-y-6 text-center md:text-left">
            <div>
            ...
            <p className="text-gray-300 text-base sm:text-lg max-w-md mx-auto md:mx-0 font-futuristic">
              ...
              <button 
                className="futuristic-button text-sm sm:text-base py-1.5 sm:py-2 cursor-pointer"
                onClick={() => {
                  console.log('ÇÖZÜMLERİMİZ butonuna tıklandı, çözümlerimiz bölümüne gidiliyor...'); // Debug
                  scrollToSection('cozumlerimiz');
                }}
              >
                <span className="pointer-events-none">ÇÖZÜMLERİMİZ</span>
              </button>
              <button 
                className="bg-accent-blue text-dark-bg hover:brightness-110 font-display uppercase text-sm sm:text-base py-1.5 sm:py-2 px-4 sm:px-6 rounded-sm transition-all duration-300 tracking-wider cursor-pointer"
                onClick={() => {
                  console.log('DEMO TALEP ET butonuna tıklandı, iletişim bölümüne gidiliyor...'); // Debug
                  scrollToSection('iletisim');
                }}
              >
                <span className="pointer-events-none">DEMO TALEP ET</span>
              </button>
            </div>
          </div>

          <div className="hidden md:block">
            {/* Bu alan boş bırakılarak 3D arka plan görünür */}
          </div>
        </div>
